...
<title>Congworks for Personal-集合住宅の区域</title>
<?php
...
<div>
<?php
exec("cmd.exe /c start requestA.bat $congnum $UIDS");
$body=file_get_contents("c:\\xampp\\htdocs\\congworks\\requestA.txt");
...
if ($body == "No System")
	...
	if (count($tbl)<2)
		...
		for($i=0;$i<count($tbl)-1;$i++)
			...
			$s=$s."<td>$line[2]</td>";
			$line[2]=str_replace(" ","_",$line[2]);
			$s=$s."<td align=right>$line[4]</td>";
			$s=$s."<td align=center>$line[5]日前</td>";
...
function Preview(num,seq,name)
	...
	PleaseWait();
	window.location.replace("viewA.php");
	}
function BackToMenu()
	{
	if (preload) return;
